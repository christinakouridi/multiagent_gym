...
class Switch(gym.Env):
    ...
    def __init__(self, full_observable=False, step_cost=0, n_agents=4, max_steps=50):
        ...
        # agent pos (2)
        self._obs_high = 1. # EDITED HERE
        self._obs_low = 0. # EDITED HERE
        if self.full_observable:
            self._obs_high = np.tile(self._obs_high, self.n_agents)
            self._obs_low = np.tile(self._obs_low, self.n_agents)
        # EDITED HERE
        self.observation_space = MultiAgentObservationSpace([spaces.Box(self._obs_low, self._obs_high, shape=(3,)) for _ in range(self.n_agents)])
